...
function getOperationSuffix(
  ...
  const { omitOperationSuffix = false, dedupeOperationSuffix = false } =
    config || {};
  const operationName =
...
> = (schema, documents, config) => {
  ...
  const operationResultSuffix = getConfigValue(
    config.operationResultSuffix,
    ""
  );
  ...
  let prepend = [];
  prepend.push(`import { kitQLClient } from '$graphql/kitQL';`);
  prepend.push(
    `import { defaultStoreValue, RequestStatus, type RequestParameters, type RequestResult } from '@kitql/client/kitQLClient';`
  );
