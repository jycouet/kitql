...
export const PAGE_match_id_ab = (params: { id: (ExtractParamType<typeof import('../params/ab.js').match>), lang?: ('fr' | 'en' | 'hu' | 'at' | string) }) => {
  return `${params?.['lang'] ? `/${params?.['lang']}`: ''}/match/${params['id']}`
}
export const PAGE_match_id_int = (params: { id: (number), lang?: ('fr' | 'en' | 'hu' | 'at' | string) }) => {
  return `${params?.['lang'] ? `/${params?.['lang']}`: ''}/match/${params['id']}`
}
export const PAGE_match_num_intJSDoc = (params: { num: (ExtractParamType<typeof import('../params/intJSDoc.js').match>), lang?: ('fr' | 'en' | 'hu' | 'at' | string) }) => {
  return `${params?.['lang'] ? `/${params?.['lang']}`: ''}/match/${params['num']}`
}
export const PAGE_match_another_id_int = (params: { id: (ExtractParamType<typeof import('../params/int.js').match>), lang?: ('fr' | 'en' | 'hu' | 'at' | string) }) => {
...
export const PAGE_u_d83e_u_dd2a = `/🤪`
export const PAGE_u_d83e_u_dd2a_emoji_u_2b50 = (params: { emoji: (string | number) }) => {
  return `/🤪/${params['emoji']}/⭐`
}
export const PAGE_x_2e_well_known = `/.well-known`
export const PAGE_a_rest_z = (params: { rest: (string | number)[] }) => {
  return `/a/${params['rest']?.join('/')}/z`
}
export const PAGE_anchors = (params: { hash: ("section0" | "section1" | "section2" | "section3") }) => {
...
export const PAGE_mdsvex = `/mdsvex`
...
export const SERVER_GET_x_2e_well_known = `/.well-known`
 ...
 * LINKS
 */
export const LINK_bluesky = `https://bsky.app/profile/jyc.dev`
export const LINK_bluesky_post = (params: { did: (string | number), post_id: (string | number) }) => {
...
export type KIT_ROUTES = {
  PAGES: { '_ROOT': never, 'base': 'all', 'subGroup': never, 'subGroup_user': never, 'subGroup2': never, 'contract': 'lang', 'contract_id': 'id' | 'lang', 'gp_one': 'lang', 'gp_two': 'lang', 'main': 'lang', 'match_id_ab': 'id' | 'lang', 'match_id_int': 'id' | 'lang', 'match_num_intJSDoc': 'num' | 'lang', 'match_another_id_int': 'id' | 'lang', 'site': 'lang', 'site_id': 'lang' | 'id', 'site_contract_siteId_contractId': 'siteId' | 'contractId' | 'lang', 'u+d83e_u+dd2a': never, 'u+d83e_u+dd2a_emoji_u+2b50': 'emoji', 'x+2e_well_known': never, 'a_rest_z': 'rest', 'anchors': never, 'anchors_id': 'id', 'lay_normal': never, 'lay_root_layout': never, 'lay_skip': never, 'md': never, 'mdsvex': never, 'sp': never, 'spArray': never, 'spArrayComma': never }
