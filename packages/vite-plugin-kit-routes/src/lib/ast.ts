				...
				if (node.type === 'ExportNamedDeclaration') {
					...
					if (specifiers) {
						specifiers.forEach((specifier) => {
							const exported = specifier.exported as any
							const exportedName = exported.name || exported.value
							if (exportedName) {
								exportedNames.push(String(exportedName))
							}
						})
					}
					...
					if (exportName) {
						...
						if (specifiers) {
							specifiers.forEach((specifier) => {
								const exported = specifier.exported as any
								const exportedName = exported.name || exported.value
								if (exportedName === exportName && specifier.local) {
									const local = specifier.local as any
									result = local.name
								}
							})
						}
