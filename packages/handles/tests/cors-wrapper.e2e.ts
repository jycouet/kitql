...
test.describe('CORS endpoint with no options', async () => {
  ...
  test('OPTIONS /api/cors-wrapper/default-options/with-response-headers', async ({ request }) => {
    const response = await request.fetch(
      '/api/cors-wrapper/default-options/with-response-headers',
      {
        method: 'OPTIONS',
      },
    )
  ...
  test('GET /api/cors-wrapper/default-options/with-response-headers', async ({ request }) => {
    const response = await request.get('/api/cors-wrapper/default-options/with-response-headers')
    expect(response.status()).toBe(200)
    expect(response.headers()['access-control-allow-origin']).toBe('*')
    expect(response.headers()['access-control-allow-methods']).toBeUndefined()
    expect(response.headers()['access-control-allow-headers']).toBeUndefined()
...
test.describe('CORS endpoint with complex options', async () => {
  ...
  test('OPTIONS /api/cors-wrapper/complex-options with non-matching origin', async ({
    request,
  }) => {
    const response = await request.fetch('/api/cors-wrapper/complex-options', {
      method: 'OPTIONS',
...
test.describe('CORS endpoint, testing edge cases', async () => {
  test('OPTIONS /api/cors-wrapper/edge-cases, no origin', async ({ request }) => {
    const response = await request.fetch('/api/cors-wrapper/edge-cases', {
      method: 'OPTIONS',
      headers: { 'Access-Control-Request-Headers': 'X-Custom-Header', Origin: 'http://google.com' },
    })
