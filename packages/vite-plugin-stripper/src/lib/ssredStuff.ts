								...
								options.properties.forEach((prop: any) => {
									if (config.args_1?.some((c) => c.fn === prop.key.name)) {
										if (prop.value && prop.value.body && prop.value.body.body) {
											...
											if (!shouldExclude) {
												// Check if the body already has an if(import.meta.env.SSR) statement to avoid nesting
												const alreadyHasSSRCheck = originalBody.length === 1 &&
													originalBody[0].type === 'IfStatement' &&
													originalBody[0].test &&
													originalBody[0].test.type === 'MemberExpression' &&
													originalBody[0].test.object &&
													originalBody[0].test.object.type === 'MemberExpression' &&
													originalBody[0].test.object.object &&
													originalBody[0].test.object.object.type === 'MetaProperty' &&
													originalBody[0].test.object.object.meta &&
													originalBody[0].test.object.object.meta.name === 'import' &&
													originalBody[0].test.object.object.property &&
													originalBody[0].test.object.object.property.name === 'meta' &&
													originalBody[0].test.object.property &&
													originalBody[0].test.object.property.name === 'env' &&
													originalBody[0].test.property &&
													originalBody[0].test.property.name === 'SSR';

												// Only wrap if not already wrapped
												if (!alreadyHasSSRCheck) {
